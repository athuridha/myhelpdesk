...
export function TicketDetail() {
  ...
  if (isLoading) return <div className="p-8 text-center text-xs text-slate-400">Memuat rincian tiket...</div>
  if (!ticket) return <div className="p-8 text-center text-xs text-rose-500">Tiket tidak ditemukan.</div>

  const canEdit = user && ['super_admin', 'division_admin', 'agent'].includes(user.role)
  const isSuperOrDivAdmin = user && ['super_admin', 'division_admin'].includes(user.role)
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="relative hidden sm:block">
              ...
              <input
                placeholder="Cari job atau nomor..."
                className="pl-8 pr-4 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-800 focus:bg-white focus:outline-none focus:border-slate-400 transition-colors w-52"
              />
            </div>
            <button
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="pt-4 border-t border-slate-100 space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold shadow-xs flex items-center gap-1.5 transition-all disabled:opacity-50"
                >
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="p-5 rounded-2xl bg-white border border-slate-200/70 shadow-2xs space-y-5 sticky top-20">
            <h3 className="font-heading font-bold text-slate-900 text-sm tracking-tight border-b border-slate-100 pb-3">
              Panel Aksi & Penanganan
            </h3>

            {/* ACTION 0: Selesaikan Tiket (Complete / Mark as Resolved) */}
            {canEdit && ticket.status !== 'SELESAI' && ticket.status !== 'DITUTUP' && (
              <div className="space-y-2">
                <button
                  onClick={() => updateTicket.mutate({ status: 'SELESAI' })}
                  disabled={updateTicket.isPending}
                  className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-xs flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-50"
                >
            ...
            {canEdit && (ticket.status === 'SELESAI' || ticket.status === 'DITUTUP') && (
              <div className="space-y-2">
                <button
                  ...
                  disabled={updateTicket.isPending}
                  className="w-full py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs shadow-xs flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-50"
                >
            ...
            {canEdit && !ticket.assignee && (
              <div className="space-y-2 pt-2 border-t border-slate-100">
                <button
                  onClick={() => user && updateTicket.mutate({ assigneeId: user.id, status: 'SEDANG_DIKERJAKAN' })}
                  disabled={updateTicket.isPending}
                  className="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-xs flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-50"
                >
            ...
            {/* ACTION 2: Assign / Reassign ke Agent Lain — ONLY SHOW WHEN UNASSIGNED OR IF USER IS SUPER/DIV ADMIN */}
            {canEdit && (isSuperOrDivAdmin || !ticket.assignee) && (
              <div className="space-y-2 pt-2 border-t border-slate-100">
                ...
                <button
                  ...
                  className="w-full py-2 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 text-slate-800 text-xs font-semibold transition-colors flex items-center justify-center gap-1.5 disabled:opacity-50"
                >
            ...
            {canEdit && (
              <div className="space-y-2 pt-2 border-t border-slate-100">
                ...
                <select
                  ...
                  className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-xs font-bold focus:outline-none focus:border-slate-400"
                >
                  {STATUSES.map((s) => (
            ...
            {canEdit && (
              <div className="space-y-2 pt-2 border-t border-slate-100">
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold shadow-xs flex items-center justify-center gap-1.5 transition-colors disabled:opacity-50"
                >
